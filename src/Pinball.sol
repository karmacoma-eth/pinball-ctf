...
library LibPinball {
    event NewRand(uint16 readRand, uint32 nextRand);
    ...
    function readRand(State memory state) internal returns (uint16) {
        ...
        state.rand = nextRand;
        uint16 currentRand = uint16(nextRand >> 16);
        emit NewRand(currentRand, nextRand);
        return currentRand;
    }
}
...
contract Pinball {
    ...
    function pull(LibPinball.State memory state, uint16 dataOff, uint16 dataLen) private returns (bool) {
        ...
        uint newLocation = (state.readRand() % 100);
        state.location = newLocation;
        emit NewLocation(newLocation);
